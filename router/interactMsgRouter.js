...
const processMessages = (payload, respond) => {
    ...
    switch (args[0]) {
        ...
        case 'follow':
            ...
            new SubscribeAPI(payload.channel.id, respond).subscribe({followed, follower, reponame});
            break;
        case 'unfollow':
            [, followed, reponame] = args;
            follower = payload.user.username;
            new SubscribeAPI(payload.channel.id, respond).unsubscribe({followed, follower, reponame});
            break;
        case 'deleteRepo':
            [, reponame] = args;
            new RepoAPI(payload.channel.id, null, respond).delete({reponame});
            break;
        case 'deleteUser':
            [, username] = args;
            new UserAPI(payload.channel.id, null, respond).delete({username});
            break;
    }
};
