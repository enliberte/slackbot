...
export default class UserRouter extends BaseRouter {
    addListeners(): void {
        ...
        this.router.post('/add-user', async (req: Request, res: Response) => {
            ...
            const msg = await this.api.userMsg.getAddResultMsg(new MsgBuilder(), {channelId, username, addedByName});
            postMessage(res, msg, channelId);
        });

        this.router.post('/users', async (req: Request, res: Response) => {
            const {channel_id: channelId} = req.body;
            const msg = await this.api.userMsg.getUsersListMsg(new MsgBuilder(), channelId);
            postMessage(res, msg, channelId);
        });
